...
setup_service_monitoring() {
    ...
    # Copy syslog monitoring script
    if [ -f "alertgrams-syslog.sh" ]; then
        cp "alertgrams-syslog.sh" "$install_dir/"
        chmod 755 "$install_dir/alertgrams-syslog.sh"
        print_success "Syslog monitoring script installed"
    else
        print_warning "alertgrams-syslog.sh not found"
    fi
...
setup_manual_monitoring() {
    ...
    if [ -f "alertgrams-syslog.sh" ]; then
        ...
        # Create convenient symlink for syslog monitoring
        if [ "$install_dir" = "/usr/local/bin" ]; then
            ln -sf "$install_dir/alertgrams-syslog.sh" /usr/local/bin/alertgrams-syslog
            print_success "Created 'alertgrams-syslog' command for syslog monitoring"
        fi
    else
        print_warning "alertgrams-syslog.sh not found"
    fi
...
# Generated by AlertGrams installer v1.1.1

# Syslog monitoring every 2 minutes (immediate alerts)
*/2 * * * * root cd $install_dir && ./alertgrams-cron.sh syslog >/dev/null 2>&1
...
EOF
    ...
    printf "  - Syslog monitoring: Every 2 minutes (immediate alerts)\n"
